...
import { getConfig, saveConfig, getProfile, saveProfile, syncProfileFromJournal } from '../api.js'
...
export default function Config() {
  ...
  const toggleWorkType = (type) => {
    ...
    if (current.includes(type)) {
      update('work_type', current.filter(t => t !== type))
    } else {
  ...
  if (loading) return <div className="text-center text-slate-500 py-12">Loading config...</div>

  if (!cfg) return (
    <div className="text-center text-slate-500 py-12">
      {error ? <span className="text-red-400">{error}</span> : 'No config loaded.'}
    </div>
  )

  const inputCls = 'bg-slate-700 border border-slate-600 text-white rounded px-3 py-2 text-sm focus:outline-none focus:border-emerald-500 transition-colors'

  return (
    <div className="max-w-2xl">
      <div className="mb-6">
        <h1 className="text-white text-2xl font-bold">Search Config</h1>
        <p className="text-slate-400 text-sm mt-1">Configure job search parameters</p>
      </div>
      ...
      <div className="space-y-5">
        ...
        <div>
          <label className="block text-sm text-slate-400 mb-1">
            Score Threshold: <span className="text-white font-medium">{cfg.score_threshold ?? 50}</span>
          </label>
        ...
        <div className="flex gap-4">
          <div className="flex-1">
            ...
            <input
              ...
              min={0}
              value={cfg.min_yoe ?? ''}
              onChange={e => update('min_yoe', e.target.value === '' ? null : Number(e.target.value))}
          ...
          <div className="flex-1">
            ...
            <input
              ...
              min={0}
              value={cfg.max_yoe ?? ''}
              onChange={e => update('max_yoe', e.target.value === '' ? null : Number(e.target.value))}
              className={`${inputCls} w-full`}
              placeholder="10"
            />
          </div>
      ...
      <div className="mt-10 border-t border-slate-700 pt-6">
        <div className="mb-4">
          <h2 className="text-white text-lg font-semibold">Candidate Profile</h2>
          <p className="text-slate-400 text-sm mt-1">
            This text is sent to the AI for every job scoring call. Keep it accurate and concise.
            Use "Sync from Journal" to auto-update it with recent work.
          </p>
        </div>
        ...
        <p className="text-slate-500 text-xs mt-2">
          "Sync from Journal" reads your last 30 journal entries and asks the AI to update your profile with new skills and experience. Review before saving.
        </p>
      </div>
    </div>
  )
