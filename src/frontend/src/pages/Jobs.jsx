    ...
    return (
        <div className="mt-6 bg-slate-800 border border-slate-700 rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-medium text-slate-300">Tailor from external job</span>
                <div className="flex gap-1 bg-slate-700 rounded p-0.5">
                    <button className={tabCls("url")} onClick={() => { setMode("url"); setResult(null); setFitWarning(null); setError(null); }}>
                        URL
                    </button>
                    <button className={tabCls("jd")} onClick={() => { setMode("jd"); setResult(null); setFitWarning(null); setError(null); }}>
                        Paste JD
                    </button>
                </div>
            </div>
